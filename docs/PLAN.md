...
| Status       | P-0..P-4 complete; P-4.5-A / B.0 / B.1 / C.0 / C.1 complete; full-suite regression sweep pending before P-4.5 close |
| Maintainer   | Xin Zhou                                                                   |
...
- **Acceptance:**
  ...
  - [x] **Codec hot-path reached (encode + decode, paired requests via `generate_batch`).** A clean-room `IdentityCodec` instance whose `encode_block` / `decode_block` are instrumented with call counters reaches non-zero counts on both sides under the following paired workload on `Qwen/Qwen3-0.6B` (dual-gated on the cached 0.6B weights). Verification entry point is **`Engine.generate_batch([prompt], params, prefix_cache=shared_pc, max_batch_size=1)`**, not `Engine.generate`; the single-request `generate` path drives `SimpleKVCache` via `_drive` and does not route through `ContinuousBatcher` / `RadixPrefixCache`, so the detached-K/V hook is not reached on that path (see `silica/engine/__init__.py:91` vs `:185`). The prompt must tokenize to ≥ `2 × block_size + 1 = 33` tokens under the default Qwen3 `block_size=16`; the `+1` satisfies batcher invariant S-5 edge 1 (`max_aligned = ((len - 1) // block_size) × block_size`, see `silica/scheduler/batcher.py` ~ line 1011) so cold and repeat-hit assert the same block count. (a) **encode-side:** the cold first `generate_batch` call registers `encode_calls ≥ floor(len(prompt_tokens) / block_size) × num_layers` via the miss-path `insert_detached` → `store.register_detached` → `codec.encode_block` chain; `decode_calls == 0` on the cold request because its own inserted blocks are not yet hit-able. (b) **decode-side:** a second `generate_batch` call with the *same* `shared_pc` and the same prompt (or any prompt whose block-aligned prefix matches) registers `decode_calls ≥ floor((len(prompt_tokens) - 1) / block_size) × num_layers` via the hit-path `lookup` → `fetch_detached_blocks` → `store.fetch_detached` → `codec.decode_block` chain. Under the `len ≥ 33` requirement both counts equal `2 × num_layers` exactly. The original PLAN wording "≥ 1 call per active KV block on a single-request `Engine.generate('Hello', max_tokens=4)` run" overspecified on three axes (wrong entry point; "Hello" tokenizes to ≪ block_size producing zero aligned blocks; a single cold run fires encode only, never decode); this amendment (2026-04-21) lands in the P-4.5-C.0 opening commit. See `docs/P4_5_C_KVCODEC_OPENING.md` §8.0-8.2 for the call-site walk and the tokenization-length invariant.
  - [ ] **No regression in the 15-row bench catalog** — `python -m scripts.bench --all` under the default env-var set exits with all cache-only rows `status="ok"`; dual-gated rows still skip as before.
- **Dependencies:** P-4.
- **Status:** in-progress (A / B.0 / B.1 / C.0 / C.1 complete 2026-04-21; P-4.5 remains open until the full-suite acceptance regression sweep — Q-010 signal re-measurement, 15-row bench run — closes the remaining three Acceptance checkboxes).
...
- **v1.6.8** (2026-04-21): **P-4.5-C.1 KVCodec runtime-integration spike — implementation + tests + opening-doc §8 rewrite + Q-012.** Lands Option (B) as specified in `docs/P4_5_C_KVCODEC_OPENING.md`. Code changes confined to `silica/kvcache/store.py`; zero scheduler / budgeter / codec-module diffs. Tests: 6 new cases — 4 dual-gated on-device on Qwen3-0.6B, 2 pure-unit with synthetic tensors. Opening doc §8 rewritten in the same commit after test authoring surfaced that the initial cohort seal does not consult the prefix cache — see new Q-012 in §10 and the corresponding P-4.5 Amendment log entry.
