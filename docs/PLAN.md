...
- **Acceptance**:
  ...
  - [ ] **Numeric cross-check against vqbench**: Silica MLX-native `BlockTQCodec` 在 Qwen3.5-0.6B (或更大) 上的 per-block reconstruction error 与 vqbench NumPy 参考数字偏差 < ε (ε 在 P-4 bench 定, 量级参考 fp32↔fp16 roundoff); Qwen3.5-4B `B=64` 4-bit K+V 配置的 end-to-end PPL 与 `vqbench/REPORT.md` 基线吻合 (差异 ≤ 数值噪声)。**"faithful 重写"必须可测, 不是"看起来差不多"**
- **Dependencies**: P-4
- **Status**: planned
...
| P1 | P-5 .. P-6 | VQ KV 压缩 + weight streaming; 使大模型在 48GB 真正跑好 |
...
- **Rationale**: Phase 0 需要建立明确的目录结构; Q-004 里 Option A 是 mini-sglang 验证过的风格; 本文档所有 Phase block 也已隐式使用此路径 (如 `silica.core.request.Request`, `silica.core.sampling.SamplingParams`), 不定会导致 Phase 0 完成时文档和代码不一致。
- **Consequences**:
...
- **Options**:
  - A. 保持 P2: engine 先稳定
  - B. 上浮到 P1 末尾: P-5 / P-6 原生能力集成和 serving 层并行
