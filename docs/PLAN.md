...
| Status       | P-0..P-4 complete; Q-010 triggered at P-4 exit → P-4.5 bridge in planning  |
| Maintainer   | Xin Zhou                                                                   |
...
- **Deliverables:**
  ...
  - [ ] **P-4.5-B — Chunked prefill (minimal).** (i) `docs/P4_5_CHUNKED_PREFILL_OPENING.md` enumerating the three options with per-option scheduler touchpoints and chosen option; (ii) implementation under `silica/scheduler/batcher.py` matching the chosen option; (iii) `tests/test_bench_ttft_under_concurrency.py` (or equivalent pytest lock) asserting the TTFT ratio against the PLAN-cited isolated baseline; (iv) greedy-output bit-identity regression test comparing the chunked and unchunked paths.
  - [ ] **P-4.5-C — KVCodec runtime integration spike.** (i) `docs/P5_OPENING.md` enumerating the three integration-point options (active KV / detached prefix store / cache wrapper) with per-option memory-ownership + parity + refactor-surface notes and chosen option; (ii) the chosen integration point wired end-to-end such that an `IdentityCodec` instance is actually reached by the forward path on `Qwen/Qwen3-0.6B` single-request (grep-visible new caller site); (iii) unit test that counts `encode_block` / `decode_block` calls and asserts they are non-zero on a real forward; (iv) a pass-through memory invariant test (`IdentityCodec.resident_bytes` equals the underlying cache's byte count within ± 0 B tolerance, so a future `BlockTQCodec` plugging in the same seam is directly comparable).
- **Acceptance:**
  - [ ] **Q-010 signal returns below threshold.** On `qwen3-0.6b-ttft-under-concurrency` vs isolated `qwen3-0.6b-smoke`, the max short-row first-token offset / isolated TTFT ratio is **< 3×** over five consecutive runs on the same machine. (3× is tighter than the 5× promotion trigger to leave headroom for noise.)
  - [ ] **Chunked-prefill correctness.** On `Qwen/Qwen3-0.6B` greedy decode, the chunked path produces the same token stream as the unchunked path for a representative workload (single long prompt, `max_tokens=16`, seed fixed). Bit-identical across the whole stream, not only the first N tokens.
  - [ ] **Codec hot-path reached.** A clean-room `IdentityCodec` instance whose `encode_block` / `decode_block` are instrumented with call counters registers **≥ 1 call per active KV block** on a single-request `Engine.generate("Hello", max_tokens=4)` on `Qwen/Qwen3-0.6B` after the P-4.5-C integration. Failure to register calls fails acceptance.
...
- **Resolution (2026-04-21):** Option A's deferral clause fired. Two independent measurements against `qwen3-0.6b-ttft-under-concurrency` (1 long ~301-token prompt + 3 one-character prompts, `max_batch_size=4`, `prefix_cache=False`) vs isolated `qwen3-0.6b-smoke` on the same Qwen3-0.6B build:
  ...
  - Promote chunked prefill to a new **P-4.5 bridge phase** (see §7 P-4.5 added in v1.6.4), not to a retroactive P-2 / P-3 deliverable. P-4.5 exits with: (i) TTFT-under-concurrency ratio < 3× on the same scenario; (ii) greedy output token-by-token identical to the unchunked path; (iii) the three-option opening doc (in-cohort chunking vs cohort splitting vs admission ordering) is landed before the implementation so the scope decision is separable from the scope implementation.
...
- **v1.6.4** (2026-04-21): **P-4 exit decision sync.** P-0..P-4 complete (see `git log` for the 14 landing commits across P-4.1..P-4.4). This revision touches only documentation — no code or interface changes — and closes the books on Q-010 while opening a new bridge phase §7 P-4.5. Landing items:
  ...
  - **New §7 P-4.5 Phase 4.5 — P-4 exit bridge.** Three sub-units: P-4.5-A (this decision-sync commit), P-4.5-B (chunked prefill minimal), P-4.5-C (KVCodec runtime integration spike). Acceptance includes: TTFT-under-concurrency ratio < 3× over five runs, chunked-vs-unchunked greedy bit-identity, `encode_block` / `decode_block` registering ≥ 1 call per KV block on a live forward (closes the interface-vs-hot-path gap identified at P-4 exit), no regression across the 15-row bench catalog. P-4.5 is a bridge, not a phase in the §8 priority-tier table; P-5 dependencies move from P-4 to P-4.5.
