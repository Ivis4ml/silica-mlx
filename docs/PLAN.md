...
- **Deliverables:**
  ...
  - [ ] `silica.bench.scenarios`: short-in/long-out; long-in/short-out; concurrent shared-prefix; **TTFT-under-concurrency** (one long-prompt request co-scheduled with short-prompt requests — resolves Q-010 chunked-prefill promotion). **Partial:** P-4.1 / P-4.2a–c shipped *model-shaped* rows (`qwen3-0.6b-smoke`, `qwen3-0.6b-b1-parity`, `qwen3-0.6b-bgt1-parity`, `qwen3.5-moe-smoke`, `gemma4-moe-smoke`). *Workload-shaped* rows pending under P-4.2d.
  - [x] Unified metrics schema: TTFT, prefill tok/s, decode tok/s, resident memory, peak memory, total tokens, wall time, oracle metadata (`ScenarioResult` dataclass; "quality" column supplied by the oracle — SMOKE / B1 / BGT1 populate structured metadata keyed for JSONL).
  - [ ] Output: jsonl + markdown report. **Partial:** JSONL emit landed with P-4.1; markdown report file pending (P-4.2d scope).
...
- **Dependencies:** P-3.
- **Status:** P-4.1 runner + P-4.2a/b/c oracle core shipped (commits `efcc65e`, `34be3f0`, `1d52f4a`, `80dda0b`); remaining — workload-shaped scenarios + markdown report (P-4.2d), `TEACHER_FORCED_ARGMAX` oracle (P-4.3), `vqbench_baseline` subprocess harness (P-4.4).
