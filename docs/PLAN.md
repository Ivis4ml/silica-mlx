...
- **Acceptance:**
  ...
  - [ ] **Numeric cross-check against vqbench** — two independent thresholds, both must pass:
    ...
    - **(b) End-to-end PPL drift — online cross-check.** On the `qwen3-0.6b-wikitext-ppl-block-tq-b64-b4` bench row (Qwen3-0.6B, `BlockTurboQuantMSE B=64` 4-bit K+V), silica's codec-backed ΔPPL (`teacher_forced_chunked_nll_with_codec` via C.2) and vqbench's subprocess ΔPPL (via the P-4.4 `--vqbench-xcheck` path landed at C.6) — both on the **same** model, same three seeds `{42, 43, 44}`, same `chunk_size`, same WikiText-2 slice — must satisfy `|Δ(ΔPPL)_silica − Δ(ΔPPL)_vqbench| < 0.01` absolute **and** `|Δ%_silica − Δ%_vqbench| < 0.1%` relative (`_compute_gap_fields` in `silica/bench/runner.py`, `vqbench_epsilon = 0.01` default + `_VQBENCH_PCT_EPSILON = 0.1` module constant). The original v1.5.1 "Qwen3.5-4B vs `vqbench/REPORT.md` static baseline" gate is a **post-P-5 required follow-up**; see Notes.
