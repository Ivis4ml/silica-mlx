...
`SyntheticPrefixBlockStore` is the pair-aware layer. Its detached dict becomes (sketch — Unit 4 checklist `plans/P5_A_U4_STORE_MIGRATION.md` §5 refines this to a private `_DetachedLayer` frozen dataclass for readability; the behaviour is the same):
...
- **(a-real) real-activation half** — closed at v1.7.5 via
  ...
  rationale: `plans/P5_A_REAL_OPENING.md`. References to a
...
`plans/P5_ACCEPTANCE_SWEEP/real_activation_xcheck.md` + `.jsonl`.
Design contract: `plans/P5_A_REAL_OPENING.md`.
...
script" design has been superseded — see `plans/P5_A_REAL_OPENING.md`
...
**Evidence (2026-04-24, landed at commit `ed57be1`; raw `plans/P5_D2_INVESTIGATION/d2a_verification_3seeds.jsonl`):**
...
**Per-seed silica numbers** (raw JSONL at `plans/P5_ACCEPTANCE_SWEEP/qwen35_4b_b_static_3seeds.jsonl`):
...
**Evidence files.** `plans/P5_ACCEPTANCE_SWEEP/qwen35_4b_b_static_close.md` (full analysis); `qwen35_4b_b_static_3seeds.jsonl` (raw bench rows); `qwen35_4b_b_static_3seeds.md` (auto-generated bench report).
...
- **No per-head / per-layer calibration in default mode** — default `(head_dim, seed)` is a single Haar rotation shared across all heads and layers. v1.7.8 added per-head rotation as **opt-in** (`per_head_rotation: bool = False` on `BlockTurboQuantMSE` / `RaBitQ1Bit` / `ExtRaBitQ`; `seed * 1000 + head_idx` matches vqbench's per-head seed convention) so the per-head choice can be empirically pinned against the (4-b) D.2a 3-seed baseline before flipping the default. Default OFF preserves the closed (4-b) evidence; per-layer calibration remains a v0.2 codec enhancement.
...
- **silica P-4.5-C closed seam** — `silica/kvcache/store.py::SyntheticPrefixBlockStore(codec=...)`; `silica/kvcache/codec.py::IdentityCodec`, `CodedBlock`, `KVCodec`; `plans/P4_5_C_KVCODEC_OPENING.md` §6.2 / §8.
