...
from the bench in one command." Per `plans/P5_OPENING.md` §7(e)
...
`plans/P5_D2_INVESTIGATION/d2a_verification_3seeds.jsonl`, not in
...
uv run python scripts/bench.py \
    ...
    --report-md plans/P5_ACCEPTANCE_SWEEP/all_kv_codecs_report.md \
    2>&1 | tee plans/P5_ACCEPTANCE_SWEEP/all_kv_codecs.log
```
...
- **`all_kv_codecs.jsonl`** — 924 rows (one per scenario × codec ×
  ...
  **absent** from every row in this sweep — `--vqbench-xcheck`
  ...
  `plans/P5_D2_INVESTIGATION/d2a_verification_3seeds.jsonl` under
  Acceptance (4-b).
...
- **Admission-headroom** (`qwen3-0.6b-admission-headroom-prefix-heavy`)
  ...
  (`plans/P5_ACCEPTANCE_SWEEP/admission_headroom.{jsonl,md}`),
  not this sweep.
...
- **Populated xcheck values are owned by Acceptance (4-b).** The
  3-seed verification in `plans/P5_D2_INVESTIGATION/d2a_verification_3seeds.jsonl`
