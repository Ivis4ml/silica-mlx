...
> (`plans/PLAN.md` §10 Q-010 Resolution, 2026-04-21), compares their
...
> acceptance criteria live in `plans/PLAN.md` §7 P-4.5 (three-layer
...
- **I-1..I-5** — row lifecycle & event taxonomy
  (`plans/P2_UNIT_16C_PREP.md` §1): every row emits `token` events
...
- **B-1..B-9** — budgeter semantics
  (`plans/P2_UNIT_16D_PREP.md` §5): reservation-timing (B-8),
...
- **S-1..S-7** — prefix-cache invariants
  (`plans/P2_UNIT_16C_2_STEP_4_SKELETON.md` §4): lookup agrees with
...
- A commit. `plans/PLAN.md` is amended in the same commit as this
...
- `plans/PLAN.md` §7 P-4.5 (chunked-prefill correctness three-layer
...
- `plans/P2_UNIT_16C_PREP.md` §1 I-5 (row lifecycle invariants);
  `plans/P2_UNIT_16C_2_STEP_4_SKELETON.md` §4 S-1..S-7 (prefix-cache
  invariants); `plans/P2_UNIT_16D_PREP.md` §5 B-1..B-9 (budgeter
  invariants).
