...
| Parent docs | `plans/P3_C5_OPENING.md` §6.4; `plans/P3_C5_DRIFT_EXPERIMENT/README.md` |
...
To capture a snapshot at boundary `K * block_size` consumed, the row's forward must HAVE run a discrete operation that stops at that boundary. mlx-lm's `gated_delta_kernel` advances the recurrent state through all S timesteps inside a single layer call and writes `cache[1]` once at the end (per `plans/P3_C5_STATE_INVENTORY/README.md` §5). There is no non-intrusive way to observe the kernel's intermediate state between timesteps without either modifying the kernel itself or splitting the input so the kernel runs over `block_size` steps at a time. **Both options are equivalent to slice-prefill at some level of the stack** — there is no "pure observation" that leaves the segmentation untouched.
...
- **Decode-step capture gating** (finding C, surfaced during C5.3.1 recon): only capture decode-era snapshots for rows whose `absolute_consumed_tokens > 0`. Rows admitted via B>1 contiguous miss cohort have counter `0` — they never entered slice-regime, and their decode-era trajectory is "contiguous + decode" rather than "slice + decode". Capturing here would seed the radix tree with cross-regime snapshots and break the §4.4 byte-exact-vs-slice-oracle gate. The `> 0` clause uses the counter itself as a "did this row go through slice-regime accounting" signal — no separate flag needed.
- **`_admit_single_hit_row` seeding moved to §4.4 (C5.3.3)**: the hit-admission seeding (populate `row.recurrent_snapshots_per_block[i]` for `i ∈ [0, K)` from ancestor nodes; set `row.absolute_consumed_tokens = K * block_size`) requires `lookup_with_node` migration AND the suffix-prefill slice helper to actually be called from `_admit_single_hit_row` — both land at C5.3.3. At C5.3.1 the hit path stays at default empty/0 fields and is unreachable in production anyway (ctor guard rejects, no test-flag bypass yet). Seeding lands together with the consumer at C5.3.3 to avoid half-finished state.

**Acceptance** (C5.3.1 only — hit-admission acceptance moves to §4.4):
...
- Synthetic-adapter test on decode-step counter: drive prefill that lands on a non-block-aligned end (e.g., `2*block_size + 1` tokens — strictly less than one trailing block), then enough decode steps to roll the counter past at least one more block boundary, verifies snapshots appear at the expected absolute block indices once the decode counter crosses each boundary.
- Synthetic-adapter test on B>1 clamp: B=2 cohort prefill stays contiguous; both rows' `recurrent_snapshots_per_block` stay empty.
- Synthetic-adapter test on the `> 0` decode-capture gate: B=2 admission via contiguous prefill, then a decode step; both rows' counters stay `0` and no snapshot is captured.
- Synthetic-adapter regression: non-recurrent adapter + `prefix_cache != None` and recurrent adapter + `prefix_cache=None` both keep contiguous prefill and produce no snapshots — preserves pre-C5.3 behaviour for those configurations bit-for-bit.
...
- **Hit-admission row seeding** (relocated from §4.2 — finding A): walk the ancestor chain of `deepest_usable` via `_Node.parent` and populate `row.recurrent_snapshots_per_block[i]` for `i ∈ [0, K)` from each ancestor's `recurrent_snapshot` (may be `None` for legacy nodes — leave the key absent). Set `row.absolute_consumed_tokens = K * block_size = usable_hit_tokens` so subsequent suffix-prefill slice forwards capture at absolute indices `K, K+1, ...`. The seeding lands here because §4.2's slice helper is what consumes the counter — landing it at C5.3.1 would write inert state with no consumer.
...
**Acceptance — hit-admission row seeding (synthetic, relocated from §4.2)**:

Synthetic-adapter test on a hit-admitted row: `K=2` cached prefix (manually inserted with non-None ancestor snapshots), suffix of `2*block_size + 1` tokens (= 2 full suffix blocks + 1 trailing partial token). Verifies `row.recurrent_snapshots_per_block.keys() == {0, 1, 2, 3}` (indices 0, 1 seeded from the K=2 ancestor nodes; indices 2, 3 captured during suffix slice-prefill; the trailing 1-token chunk does not produce a snapshot). `row.absolute_consumed_tokens` ends at `K * block_size + 2 * block_size + 1`.
...
A radix tree with deep prefixes accumulates one snapshot per node. On Qwen3.5-0.8B, ~19.5MB per snapshot (per `plans/P3_C5_STATE_INVENTORY/README.md` §6). A 16-block tree × multiple unique prefix paths could pin hundreds of megabytes.
