...
- **Status (2026-05-06): Pending pre-projection; gates (i)+(ii) resolved, (iii) reframed.**
  Gate (i) verification performed 2026-05-06: result **outcome A\***
  (hybrid mixed precision); spike doc §3.5 records details. Gate (ii)
  downloads completed 2026-05-06: target
  `mlx-community/gemma-4-31b-it-4bit` (snapshot `dcb78c3`, 17 GB) +
  drafter `mlx-community/gemma-4-31B-it-assistant-bf16` (snapshot
  `28e9227`, 926 MB) cached locally. **Gate (iii) reframed
  2026-05-06.** `mlx-vlm 0.5.0` (the version with Gemma 4 MTP CLI)
  requires `mlx>=0.31.2 / mlx-lm>=0.31.3`; silica's project pin is
  `mlx==0.31.1 / mlx-lm==0.31.2 / mlx-metal==0.31.1` per the v1.7.21
  determinism anchor in `tests/test_p2_preload_parity.py` (cycle-11
  argmax flip; bisect not yet performed). `uv add --dev mlx-vlm` would
  therefore force-bump the pin and break the anchor. Decision: install
  `mlx-vlm 0.5.0` into a project-external isolated venv at
  `~/.cache/silica-d023-mtp/.venv`; `pyproject.toml` and `uv.lock`
  remain untouched. Spike doc §4 records the runtime stack divergence;
  the verdict report (§10) will record the exact installed versions
  and the divergence-statement. Gate (iv) license reconciliation
  deferred — spike does not bundle weights into `silica.*`. P-8
  (OpenAI HTTP server) opens cleanly after D-023 settles in any
  direction.
- **D-024 trigger note.** A future dependency-upgrade / bisect
  decision (covering the cycle-11 argmax flip) opens as D-024 only if
  D-023 B=1 PASS triggers the native integration ladder OR mlx 0.32+
  ships with concrete payoff (e.g., async-copy per D-022 ε waitlist);
  no D-024 entry today.
