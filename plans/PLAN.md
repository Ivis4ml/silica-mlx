...
- **Stop-and-ask gates** per the locked startup sequence (gate (i)
  resolved on 2026-05-06; gates (ii)-(iv) still pending):
  ...
  a 4-bit IT target if outcome A / A\* holds, ~17-19 GB est.); (iii) `uv add
...
- **Status (2026-05-06): Pending pre-projection; gate (i) resolved.**
  Four gates are summarised above; three remain pending. Gate (i) verification
  performed 2026-05-06 via HF model search + drafter card +
  `mlx-vlm` README; result is **outcome A\*** (hybrid mixed precision):
  `mlx-community/gemma-4-31b-it-4bit` exists as a 4-bit IT target
  (resolves the 48 GB ceiling), no precision-matched 4-bit drafter
  exists (only `mlx-community/gemma-4-31B-it-assistant-bf16` 939 MB
  bf16), and mlx-vlm does not document mixed-precision pairings.
  User authorized the 4-bit IT target + bf16 drafter pairing
  2026-05-06; precision mismatch will be recorded as a caveat in the
  verdict. Spike doc §3.5 records the verification details. Outcome B
  (BF16-only, no mixed precision) is decisively not the case here —
  the M5-Pro external spike does not need to close on
  hardware-feasibility. Gate (ii) downloads, gate (iii) `uv add --dev
  mlx-vlm`, and gate (iv) license reconciliation remain pending. P-8
  (OpenAI HTTP server) opens cleanly after D-023 settles in any
  direction.
...
- **v1.7.29** (2026-05-06): **Track C external reopen probe opened —
  ...
  **Stop-and-ask gates** before any measurement (gate (i) resolved on
  2026-05-06; gates (ii)-(iv) still pending):
  (i) **supported-pairing + hardware-feasibility verification** (the
  ...
  / A\* holds, the corresponding 4-bit IT target ~17-19 GB est.
