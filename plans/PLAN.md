...
- **v1.7.38** (2026-05-08): **silica chat — fixed-height scroll
  window for streaming reasoning blocks (`thinking="show"` UX
  fix).** The v1.7.36 default flip set `thinking="show"` so users
  could watch the reasoning rather than wait behind a static
  spinner; on Qwen3 / Qwen3.5 / Gemma 4 turns with long
  ``Thinking Process: 1. **Analyze...** 2. **Identify...**`` blocks
  the unbounded wall-of-text would scroll the prompt off the top
  of the terminal and bury the visible reply once `</think>`
  arrived. v1.7.38 routes the EnterThinking / ThinkingChunk /
  ExitThinking event stream in
  `silica/chat/cli/app.py:_stream_callback` through a new
  `silica.chat.cli.thinking_scroll.ThinkingScrollWindow` that
  renders a magenta `⠋ thinking...` header plus the most recent 6
  body lines in a fixed-height region, redrawing in place via
  ANSI cursor controls (`\x1b[NA` cursor-up + `\x1b[J`
  clear-from-cursor). On `ExitThinking` the region is wiped so the
  trailing `thought for Xs` summary lands cleanly; identical wipe
  on `KeyboardInterrupt` / exception / max-tokens-mid-thinking so
  the post-turn newline never inherits a stale rolling region.
  Gating mirrors `make_live_toolbar`: scroll path activates only
  when `thinking_display == "show"` AND `output_stream.isatty()`
  AND `TERM` ∉ {empty, dumb} AND `palette.mode != PaletteMode.PLAIN`;
  on any failure (piping to a file, `TERM=dumb`, `NO_COLOR=1`) the
  v1.7.36 wall-of-text fallback path remains in place so captured
  output stays free of cursor escapes. CJK width handling is a
  coarse East-Asian-Wide / common-emoji ord-range estimator
  (CJK / Hangul / fullwidth / 0x1F300-0x1F9FF emoji blocks → 2
  cells; everything else → 1) without a `wcwidth` dependency, with
  every rendered line truncated to `terminal_cols - 1` cells with
  appended `…` so width-exact lines cannot trigger Terminal.app's
  auto-wrap-on-final-column and de-sync the cursor-up arithmetic.
  `thinking_mode` / `thinking` / `thinking_display` semantics are
  unchanged (`thinking="hidden"` still suppresses the entire path,
  `thinking="auto"` still shows only the indicator + summary, the
  v1.7.37 thinking_mode default of `True` is preserved). No new
  config knob is added — the per-turn `ThinkingScrollWindow`
  construction in `silica/chat/cli/app.py` (alongside the
  per-turn `ThinkingParser` reset, line ~830) takes its
  defaults from the helper. v1.7.36's max_tokens 1024→8192 raise,
  thinking="show" display default, and tightened
  DEFAULT_SYSTEM_PROMPT all stay in place — they are
  correctness-orthogonal to this UX fix. v1.7.37's sliding-attention
  prefix-cache gate and thinking_mode revert also stay in place
  unchanged. Helper landed in commit A `<sha>` (332-line module +
  445-line test file with 33 unit pins covering cell-width
  estimator, truncation, gating predicates, no-op-disabled mode,
  render protocol against a fake palette / fake stream, CJK
  rendering, and `max_lines` clamping); commit B `<sha>` wires
  the helper into `silica/chat/cli/app.py` (import + per-turn
  construction + EnterThinking / ThinkingChunk / ExitThinking
  dispatch + KeyboardInterrupt / exception / natural-end clear)
  and lands the PLAN.md §13 v1.7.38 entry. Test sweep at commit B:
  192 passed across `tests/test_chat_cli_thinking_scroll.py` (33
  pins, helper-only) + `tests/test_chat_cli_app.py` + 
  `tests/test_chat_cli_thinking_parser.py` + 
  `tests/test_chat_cli_config.py`, no regressions in the existing
  app-renderer surface (the renderer tests do not assert on
  EnterThinking / ThinkingChunk / ExitThinking ANSI specifics, so
  the dispatch fork is non-breaking by inspection and confirmed
  empirically).

- **v1.7.37** (2026-05-07): **silica chat post-v1.7.36 corrections
  — sliding-attention prefix-cache gate (Gemma 4 unblocked) and
  ``thinking_mode`` default revert (Qwen3.5-A3B + Gemma 4 unbroken).**
  Two independent bugs surfaced by the v1.7.36 default flips,
  fixed in one commit because both are post-v1.7.36 corrections
  and reverting separately would leave the tree in an awkward
  intermediate state. Neither fix touches the v1.7.36
  ``max_tokens=8192`` raise, the ``thinking="show"`` display
  default, or the tightened ``DEFAULT_SYSTEM_PROMPT`` — those
  are correctness-orthogonal to both bugs.

  ## Bug 1 — sliding-attention prefix-cache gate (Gemma 4
  unblocked)

  A user-reported regression on
  `silica chat --model mlx-community/gemma-4-31b-4bit`: the very
  ...
  AttentionKind.SLIDING (attention_kinds=['global', 'sliding'])``
  during prefill. The chat REPL was unconditionally constructing
  a `RadixPrefixCache` and threading it into `ChatSession`;
  `silica/scheduler/batcher.py:294` rejects that combination on
  sliding-bearing adapters because the seed path emits
  `BatchKVCache` per layer and the window-truncation / offset /
  rotated semantics of `BatchRotatingKVCache` under seeded
  admission are not yet validated (P-3-D3 follow-up). The error
  message itself prescribed the workaround: pass
  `prefix_cache=None` to fall through to the miss-only path.

  **Fix.** `silica/chat/cli/app.py:_build_prefix_cache` now
  inspects `adapter.capabilities().attention_kinds` and returns
  `None` when ``"sliding"`` is present. The session then runs
  on the miss-only path: `ChatSession` with `prefix_cache=None`
  re-prefills the full conversation history each turn (no
  per-turn RadixPrefixCache reuse). The chat-bench harness's
  parallel `silica/bench/chat_bench.py:_default_cache_builder`
  (which the docstring explicitly marks KEEP-IN-SYNC with the
  REPL builder) gets the same gate so running chat-bench
  against Gemma 4 does not trip the same error.

  **User notice.** The REPL prints a one-line dimmed-grey notice
  at session construction when the cache is disabled —
  ``[note: sliding-attention model; multi-turn prefix caching
  disabled]`` — so the user understands why subsequent turns
  re-prefill from scratch instead of reusing prior-turn KV.
  Reset / load / model-swap paths print nothing (the user
  already heard the notice once for this model).

  **What still works on Gemma 4.** Single-turn generation,
  multi-turn chat, all `/config` keys, `/system`, `/regenerate`,
  `/continue`, `/save` / `/load`, `/model` swap. The only
  missing capability is per-turn prefix-cache reuse;
  conversation history is correct turn-to-turn, just slower
  per-turn TTFT.

  **What is NOT changed.** The `silica.scheduler.batcher`
  guard at line 294 itself — that is the right guard; the
  long-term fix is the P-3-D3 follow-up that validates the
  `BatchRotatingKVCache` seeded-admission path (out of v1.0
  scope, tracked in `plans/PLAN.md` §10 and the existing
  P-3-D3 reference). The `ChatSession` `prefix_cache` API
  remains `None`-tolerant unchanged. Other adapters (Qwen3 /
  Qwen3.5 / Qwen3-Next dense + MoE) still attach a real
  `RadixPrefixCache` and exercise the cached path unchanged.

  **Test pin updates** (5 new pins, all paths under `tests/`):
  - **New** `test_chat_cli_app.py::test_build_prefix_cache_returns_none_for_sliding_attention`
    — locks the gate behaviour (sliding-bearing adapter →
    `_build_prefix_cache` returns `None`).
  - **New** `test_chat_cli_app.py::test_build_prefix_cache_short_circuits_before_codec_factory`
    — proves the short-circuit happens BEFORE the codec
    factory runs so we do not waste mlx allocations on a cache
    the caller throws away.
  - **New** `test_chat_cli_app.py::test_build_prefix_cache_global_only_adapter_still_builds_cache`
    — negative-side guard: pure-global adapters (Qwen3 /
    Qwen3.5) must still construct a real cache, so a polarity
    flip in the gate cannot silently disable prefix caching
    everywhere.
  - **New** `test_chat_bench.py::test_default_cache_builder_returns_none_for_sliding_attention`
    — chat-bench parity for the same gate.
  - **New** `test_chat_bench.py::test_default_cache_builder_returns_cache_for_global_only_attention`
    — chat-bench parity for the negative-side guard.

  **Test status.** 413 tests passing across `test_chat_cli_app.py`,
  `test_chat_cli_config.py`, `test_chat_cli_toolbar.py`,
  `test_chat_cli_thinking_scroll.py`,
  `test_chat_session.py`, `test_chat_cli_commands.py`, and
  `test_chat_bench.py` (the chat-affected surface).

  **References (Bug 1).** `silica/chat/cli/app.py` (gate at the
  top of `_build_prefix_cache` + notice in `run_chat`);
  `silica/bench/chat_bench.py:_default_cache_builder` (parity
  gate + KEEP-IN-SYNC docstring update);
  `silica/scheduler/batcher.py:294` (the original guard that
  fired); `silica/models/adapter.py:112-126` (the
  `AttentionKind` enum); `tests/test_chat_cli_app.py` and
  `tests/test_chat_bench.py` (the five new pins). The user's
  reported session crashed on
  `silica chat --model mlx-community/gemma-4-31b-4bit`; after
  this fix, `silica chat` on that model returns the prompt
  successfully and prints the dimmed-grey note about
  multi-turn caching being disabled.

  ## Bug 2 — ``thinking_mode`` default revert (v1.7.36
  ``True → False`` reverted to ``True``)

  The v1.7.36 flip of ``CONFIG_SCHEMA["thinking_mode"].default``
  from ``True`` to ``False`` (chat-template ``enable_thinking``
  propagation) was over-correction. Live testing on
  ``mlx-community/Qwen3.5-35B-A3B-4bit`` and
  ``mlx-community/gemma-4-31b-4bit`` showed the model emitting
  literal ``<|im_end|>`` text after (or instead of) the answer
  and free-completing into hallucinated additional turns
  (system prompt re-emission, fake user turns, fake assistant
  code) until the v1.7.36 ``max_tokens=8192`` ceiling was hit.
  Signature is unambiguous: the model's first decoded tokens
  decode to ``"<|im_end|>\n<|im_start|>system\n..."`` rather
  than the actual reply — the model is in text-completion mode,
  treating the chat template as plain text and continuing the
  pattern. Qwen3.5-A3B was almost certainly fine-tuned with
  thinking-on as the dominant distribution and the chat
  template's ``else`` branch (empty ``<think>\n\n</think>``
  filler when ``enable_thinking=False``) is OOD enough that
  the model does not recover. The verbose-reasoning UX the
  flip aimed to address is best handled by the v1.7.38
  fixed-height
  :class:`silica.chat.cli.thinking_scroll.ThinkingScrollWindow`
  (already drafted, helper + 33 unit tests in the working
  tree) — display the last 6 lines of reasoning text in a
  rolling window rather than disabling reasoning entirely.

  **Three coordinated edits, plus the v1.7.36 test pin's
  polarity flipped:**
  - `silica/chat/cli/config.py CONFIG_SCHEMA["thinking_mode"].default`:
    ``False → True``; ``valid_help`` rewritten to put ``on``
    as default and ``off`` as opt-in, with a one-sentence note
    about the v1.7.36 → v1.7.37 history.
  - `silica/chat/cli/config.py` module docstring: amended to
    record the v1.7.36 → v1.7.37 sequence (two adjustments
    retained, one reverted) so a future reader of the docstring
    sees the canonical history rather than just the surviving
    state.
  - `silica/chat/cli/app.py:785`
    (``state.config.get("thinking_mode", False)``): ``False
    → True`` so the defensive fallback agrees with the schema
    default if the key is ever missing from ``state.config``.
  - `tests/test_chat_cli_config.py`: the v1.7.36 pin
    ``test_thinking_mode_default_is_false`` is renamed to
    ``test_thinking_mode_default_is_true`` and the assertion
    body inverts. The pin is load-bearing in the new direction
    so a copy-paste regression cannot silently re-flip the
    default.

  **References (Bug 2).**
  `silica/chat/cli/config.py:243-260` (thinking_mode
  CONFIG_SCHEMA entry, the v1.7.37 default revert lives here)
  + `:23-44` (module docstring with the v1.7.36 → v1.7.37
  reasoning); `silica/chat/cli/app.py:785` (defensive fallback
  alignment); `tests/test_chat_cli_config.py` (the renamed pin).
  Live evidence captured in the user's chat transcript on
  2026-05-07 for both Qwen3.5-A3B-4bit (text-completion-mode
  output starting with ``<|im_end|>\n<|im_start|>system\n``
  re-emitting the silica system prompt + a fabricated Python
  tutorial turn) and Gemma 4 (correct first-turn answer
  followed by ``<|im_end|>`` + fabricated turns under the
  ``thinking_mode=on`` test variant — see Known limit below
  for the residual stop-token issue on Gemma 4 mlx-community).

  ## Known limit — Gemma 4 (mlx-community) stop-token detection
  (deferred follow-up)

  The user's verification test under ``thinking_mode=on`` on
  ``mlx-community/gemma-4-31b-4bit`` produced a correct first
  answer to the test prompt, then continued generating
  ``<|im_end|>``-delimited fabricated turns. Tokenizer
  inspection shows this checkpoint uses a non-standard
  harmony-style special-token schema: ``<|turn>`` (id 105) and
  ``<turn|>`` (id 106) are special tokens; ``<|im_end|>`` is
  NOT a single special token (it tokenizes to 6 normal tokens
  ``[236820, 236909, 548, 236779, 643, 111038]``);
  ``<end_of_turn>`` is also NOT a special token (7 normal
  tokens); the only tokenizer-declared EOS is ``<eos>`` (id 1).
  ``tokenizer.chat_template`` is the empty string, so silica's
  :meth:`silica.chat.session.ChatSession._render_prompt` falls
  through to its Qwen-style manual fallback at
  ``silica/chat/session.py:1170-1178`` which renders
  ``<|im_start|>{role}\n{content}<|im_end|>``. The model has
  evidently been fine-tuned on enough Qwen-format data to
  understand the prompt and produce the correct first answer,
  but it then attempts to mark turn end with ``<|im_end|>`` —
  which decomposes to 6 ordinary text tokens and is not in the
  ``stop_token_ids`` set, so generation continues to the
  ``max_tokens`` ceiling. v1.7.37 does NOT fix this: a clean
  fix requires either (a) a model-aware chat-template selector
  (Qwen vs Gemma vs harmony), (b) string-pattern stop sequences
  added at the engine layer (``SamplingParams.stop`` is declared
  but unused — ``silica/core/sampling.py:36``), or (c)
  registration of ``<|im_end|>``-equivalent tokens for the
  fallback path; all three are out of v1.0 scope. Recommended
  workaround for v1.0: use Qwen3 / Qwen3.5 family models for
  multi-turn chat (Gemma 4 mlx-community variant prints a
  correct first-turn answer but does not terminate cleanly,
  so the user sees the answer immediately followed by
  fabricated continuation; ``Ctrl+C`` aborts the generation
  cleanly).
