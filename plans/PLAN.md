...
- **Pairing + feasibility caveat at the top.** The advertised MTP pair
  ...
  cached `mlx-community/gemma-4-31b-4bit` (17 GB) is the **non-IT
  4-bit variant** and is **not the supported MTP target**. The
  advertised BF16 target also exceeds the M5 Pro 48 GB unified-memory
  ceiling, so the spike is not runnable as-advertised on this hardware.
  Whether D-023 can run at all depends on a supported-pairing
  verification (does an MTP-supported 4-bit IT target variant exist?
  does mlx-vlm support mixed precision?) which is the load-bearing
  pre-spike step. See `plans/MTP_GEMMA4_PRE_PROJECTION.md` §3.
...
- **Four explicit stop-and-ask gates** per the locked startup sequence:
  ...
  PAIR-INFEASIBLE in the spike doc); (ii) drafter download
  (`mlx-community/gemma-4-31B-it-assistant-bf16` ~939 MB; plus possibly
  a 4-bit IT target if outcome A holds, ~17-19 GB est.); (iii) `uv add
  --dev mlx-vlm` (dev-only; do not pollute mainline deps; re-verify
  the `mlx==0.31.1 / mlx-lm==0.31.2 / mlx-metal==0.31.1` pin afterwards);
  (iv) license reconciliation (待核查 — official Apache-2.0 vs
  `mlx-community` conversion-metadata `License: gemma`).
...
- **Status (2026-05-06): Pending pre-projection.** Four stop-and-ask
  gates summarised above. Measurement begins after gate (i) resolves
  to outcome A (4-bit IT pair exists) or outcome C (mixed-precision
  supported with explicit user authorization); outcome B (BF16-only,
  no mixed precision) closes this M5-Pro external spike with
  hardware-feasibility negative before any download or measurement,
  and the broader Gemma 4 MTP question stays open under monitoring
  for a future 4-bit IT target conversion or documented mlx-vlm
  mixed-precision support; outcome D (cloud / remote box) is out of
  scope for this half-day external probe and defers. P-8 (OpenAI HTTP
  server) opens cleanly after D-023 settles in any direction.
...
- **v1.7.29** (2026-05-06): **Track C external reopen probe opened —
  ...
  **Pairing + feasibility caveat at the top.** The advertised MTP pair
  ...
  (~939 MB BF16). The cached `mlx-community/gemma-4-31b-4bit` (17 GB)
  is the **non-IT 4-bit variant** and is **not the supported MTP target**.
  The advertised BF16 target also exceeds the M5 Pro 48 GB unified-memory
  ceiling, so the spike is not runnable as-advertised on this hardware.
  Whether D-023 can run depends on a supported-pairing verification
  (does an MTP-supported 4-bit IT target variant exist? does mlx-vlm
  support mixed precision?) which is recorded in the spike doc §3 as
  the load-bearing pre-spike step with four outcomes A/B/C/D.
  ...
  **Four stop-and-ask gates** before any measurement:
  (i) **supported-pairing + hardware-feasibility verification** (the
  load-bearing pre-spike step per the caveat above). Spike doc §3
  enumerates four outcomes — A: 4-bit IT pair exists, ~17-19 GB total,
  spike is feasible; B: only BF16 pair exists, mixed-precision
  ...
  (ii) drafter / target download authorization — drafter
  `mlx-community/gemma-4-31B-it-assistant-bf16` ~939 MB; if outcome A
  holds, the corresponding 4-bit IT target ~17-19 GB est.
  (iii) `mlx-vlm` not installed — install via `uv add --dev mlx-vlm`
  (dev-only, must not pollute mainline `[project].dependencies`) and
  re-verify the `mlx==0.31.1 / mlx-lm==0.31.2 / mlx-metal==0.31.1` pin
  is intact afterwards.
  (iv) license reconciliation — official Apache-2.0 (per Google blog
  and `ai.google.dev/gemma/docs/mtp/overview`) vs `mlx-community`
  conversion-metadata `License: gemma`. Reuse decision is deferred until
  reconciled; the spike itself does not require the reuse decision
  because no model files are bundled into `silica.*`.
