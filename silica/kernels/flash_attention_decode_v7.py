"""FA-decode v7: v6 + vectorized half4 K/V loads.

v6 used scalar half loads/stores against threadgroup memory. v7 uses
metal::half4 vector loads (8 bytes / load) to coalesce HBM reads.

Q is small (8 KB total at production shape) so left scalar; K and V are
the bulk of bandwidth so vectorized.
"""
...
_FORWARD_SOURCE = """
    ...
    constexpr uint D_PER_T = D / TPS;        // = 8 scalar halfs
    constexpr uint D_PER_T_VEC = D_PER_T / 4; // = 2 half4 vectors per thread per row
    ...
    // Vectorized device pointers as half4 — kv data is contiguous in (B*H_kv*T_kv*D) layout.
    ...
    while (k_pos < k_end) {
        ...
        // Cooperative half4 load: TK=32 rows × D_VEC=64 vec cells = 2048 vec cells.
        // 192 threads -> ~10.7 cells per thread.
        for (uint vec_idx = tid_in_tg; vec_idx < TK * D_VEC; vec_idx += THREADS_PER_TG) {
            ...
            if (row < tk_actual) {
                ...
                // Store as 4 scalars to keep the rest of the kernel simple.
                uint col4 = vcol * 4;
...
def _build_fwd() -> object:
    return mx.fast.metal_kernel(
        name="silica_flash_attention_decode_v7_fwd",
...
def _build_merge(has_gate: bool) -> object:
    return mx.fast.metal_kernel(
        name=f"silica_flash_attention_decode_v7_merge_{'gated' if has_gate else 'plain'}",
...
def flash_attention_decode_v7(
    ...
    if H_q // H_kv != 6:
        raise NotImplementedError("v7 hardcodes Q_PER_KV=6")
    if scale is None:
        scale = D ** -0.5
...
__all__ = ["flash_attention_decode_v7"]
