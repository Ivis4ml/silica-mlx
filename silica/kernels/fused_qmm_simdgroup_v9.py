"""4-bit QMM kernel v9 — v4 minus aggressive unroll + prefetch hints.

Cycle 8 fourth iteration. v4 was best at 0.83 ms. v8 (8 N-tiles)
regressed to 1.24 due to register pressure. v7 (2 sg/tg) regressed to
0.89. The structural sweet spot is empirically 4 N-tiles, 1 simdgroup.

v9 keeps v4's structure but tries two compiler hints:
1. Remove `#pragma unroll` on inner N-tiles loop — let compiler decide
   (sometimes manual unroll causes register spills the compiler avoids)
2. Add explicit prefetch via __builtin_prefetch on next group's
   scale/bias and on next K_TILE's weights — Apple Silicon prefetcher
   is aggressive but explicit hints can cover specific patterns
"""
...
_KERNEL_SOURCE = """
    ...
    // Pre-load first group's scale/bias outside the loop (eliminates branch
    // in initial iteration; allows compiler to schedule).
    ...
    for (uint group_base = 0; group_base < K_val; group_base += G) {
        ...
        // Pre-load NEXT group's scale/bias before continuing (overlap with
        // ongoing MMA work). Skip on last group iteration.
...
def _build_kernel() -> object:
    return mx.fast.metal_kernel(
        name="silica_fused_qmm_simdgroup_v9",
...
__all__ = ["fused_qmm_simdgroup_v9"]
