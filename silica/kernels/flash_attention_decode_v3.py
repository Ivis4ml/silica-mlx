"""FA-decode v3: GQA-aware sharing with separate K/V tile buffers.

v2 reused threadgroup memory for K then V, requiring a per-thread p_local[32]
register array between stages. That blew register budget at B≥16 and
corrupted output. v3 keeps K and V in separate threadgroup buffers (like v1)
so we can compute p inline without storing it.

Threadgroup memory: 32 KB (16 KB k_tile + 16 KB v_tile, both half-precision).
"""
...
_KERNEL_SOURCE = """
    ...
    while (k_pos < T_kv) {
        ...
        // Cooperative load of BOTH K and V (192 threads).
        // Total cells: 2 * TK * D = 16384. Each thread loads ~85 cells.
        ...
        // Online-softmax update + weighted V accumulate, all inline (no p_local
        // register array).
        float m_new = m_cur;
        for (uint k_idx = 0; k_idx < TK; k_idx++) {
...
def _build_kernel(has_gate: bool) -> object:
    return mx.fast.metal_kernel(
        name=f"silica_flash_attention_decode_v3_{'gated' if has_gate else 'plain'}",
...
def flash_attention_decode_v3(
    ...
    *,
    ...
    if H_q // H_kv != 6:
        raise NotImplementedError(f"v3 hardcodes Q_PER_KV=6; got {H_q}/{H_kv}")
    if scale is None:
        scale = D ** -0.5

    has_gate = gate is not None
    if has_gate and gate.shape != q.shape:
        raise ValueError(f"gate shape mismatch")
...
__all__ = ["flash_attention_decode_v3"]
