"""4-bit QMM kernel v10 — 2-K_TILEs-per-barrier batching.

Cycle 8 v10. v9 was new best at 0.58-0.60 ms (1.27× slower than mlx).
v10 keeps v9's compiler-friendly structure but groups 2 K_TILEs per
barrier — 4 barriers per group instead of 8, and lets 2 MMAs run
back-to-back.

Trade vs v9:
    - 4 vs 8 barriers per group (50% fewer threadgroup syncs)
    - 2× threadgroup memory for b_tiles (still small, 2 KB)
    - 8 simdgroup_load + 8 MMA per group (was 4 load + 4 MMA in v9)
"""
...
_KERNEL_SOURCE = """
    ...
    constexpr uint K_BATCH = 2;  // dequantize 2 K_TILEs at a time
    ...
    // 2 K_TILEs per dequant batch → b_tiles holds 2 sets of [N tiles][N row][K col]
    threadgroup half b_tiles[K_BATCH][N_TILES_PER_SG][N_TILE][K_TILE];
    ...
    for (uint group_base = 0; group_base < K_val; group_base += G) {
        ...
        // Process the group as 4 batches of 2 K_TILEs each.
        for (uint t_batch = 0; t_batch < TILES_PER_GROUP; t_batch += K_BATCH) {
            // Dequantize 2 K_TILEs worth of data into b_tiles[0..1][nb][row][col]
            for (uint kb = 0; kb < K_BATCH; kb++) {
                uint t = t_batch + kb;
                ...
                for (uint nb = 0; nb < N_TILES_PER_SG; nb++) {
                    ...
                    b_tiles[kb][nb][row0][col0] = half(nibble0) * scale_r0[nb] + bias_r0[nb];
                    b_tiles[kb][nb][row1][col1] = half(nibble1) * scale_r1[nb] + bias_r1[nb];
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);

            // Run 2 MMA passes back-to-back
            for (uint kb = 0; kb < K_BATCH; kb++) {
                uint t = t_batch + kb;
                uint k_base = group_base + t * K_TILE;
                ...
                simdgroup_load(B0, &b_tiles[kb][0][0][0], K_TILE, ulong2(0, 0), true);
                simdgroup_load(B1, &b_tiles[kb][1][0][0], K_TILE, ulong2(0, 0), true);
                simdgroup_load(B2, &b_tiles[kb][2][0][0], K_TILE, ulong2(0, 0), true);
                simdgroup_load(B3, &b_tiles[kb][3][0][0], K_TILE, ulong2(0, 0), true);
        ...
        // Prefetch next group's scale/bias.
...
def _build_kernel() -> object:
    return mx.fast.metal_kernel(
        name="silica_fused_qmm_simdgroup_v10",
...
__all__ = ["fused_qmm_simdgroup_v10"]
